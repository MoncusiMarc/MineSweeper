...
class minesweeper {
    ...
    calculateMines() {
        ...
        this.board.forEach((row) => {
            row.forEach((cell) => {
                if (cell.content == '*') tempMines++
                if (cell.state == 'flagged') tempMines--
            })
        })
    ...
    openAdjacentCells(row, col) {
        for (let i = row - 1; i <= row + 1; i++) {
            for (let j = col - 1; j <= col + 1; j++) {
                ...
                if (this.board[i][j].opened) continue
                if (this.board[i][j].state == 'flagged') continue
                this.openCell(i, j)
            }
        }
    }
    ...
    countAdjCells(row, col) {
        ...
        for (let i = row - 1; i <= row + 1; i++) {
            for (let j = col - 1; j <= col + 1; j++) {
                if (i < 0 || i >= this.rows) continue
                if (j < 0 || j >= this.cols) continue
                if (this.board[i][j].content == '*') value++
            }
        }
        return value
    }
        ...
        for (let i = 0; i < this.rows; i++) {
            for (let j = 0; j < this.cols; j++) {
                if (!this.board[i][j].opened) {
                    if (this.board[i][j].content != '*') return 'none'
                }
                if (this.board[i][j].opened) {
                    if (this.board[i][j].content == '*') return 'lost'
                }
            }
        }
        return 'won'
